...
export class WebRTCWebSocketService {
  ...
  private connectionClosed$ = new Subject<void>(); //To Check
        ...
        tap({
          error: (error) => {
            ...
            this.sharedService.setToastAndNotification({
              ...
              message: "Failed to connect to WebRTC signaling server",
            });
          },
        }),
        catchError((_) => EMPTY)
  ...
  private logIncomingMessage(message: any): void {
    try {
      ...
      console.log("WebRTC message received:", dataObject.type, dataObject);
